...
一个基于 Vue 3 和 Vite 的全栈书籍推荐 Web 应用。

## 技术栈

- **前端**：Vue 3（Composition API）、Vue Router 4、Vite
- **样式**：纯 CSS（scoped）、Poppins + Lora + Inter 字体、Bootstrap 5（基础层）
- **状态管理**：Vue Composition API（ref、computed、watch）
...
├── vite.config.js
├── src/
│   ├── main.js                # Vue 入口 + createRouter（8 条路由）
...
│   ├── composables/
│   │   └── usePagination.js   # 异步分页 composable（async/await）
│   ├── components/
│   │   ├── Navbar.vue         # 全局导航栏
│   │   ├── SearchBar.vue      # 搜索输入框
│   │   ├── BookCard.vue       # 书籍卡片（3 种尺寸）
│   │   ├── TagCloud.vue       # 标签云
...
`usePagination` composable 已预留后端对接接口：
